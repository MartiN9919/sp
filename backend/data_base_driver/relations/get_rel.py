...
def get_rel_by_object(group_id, object, id, parents):
    ...
    first_data = [{'object_type': rel[2], 'rel_type': rel[0], 'rec_id': rel[3], 'params': get_object_record_by_id_http(rel[2], rel[3])['params']} for rel in rels if
                  (rel[4] == object and rel[5] == id) and len(
                      [temp for temp in parents if int(temp[0]) == rel[2] and temp[1] == rel[3]]) == 0]
    second_data = [{'object_type': rel[4], 'rel_type': rel[0], 'rec_id': rel[5], 'params': get_object_record_by_id_http(rel[4], rel[5])['params']} for rel in rels if
                   (rel[2] == object and rel[3] == id) and len(
                       [temp for temp in parents if int(temp[0]) == rel[4] and temp[1] == rel[5]]) == 0]
    return {'object_type': object, 'rel_type': 'parent', 'rec_id': id, 'params': get_object_record_by_id_http(object, id)['params'],
