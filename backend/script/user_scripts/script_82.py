...
from data_base_driver.input_output.io_geo import relations_to_geometry_id, feature_collection_by_geometry, get_geometries
...
def script_82(request, group_id):
	try:
		geometries = [item['rec_id'] for item in search({'actual': False, 'object_id': 30, 'request': '', 'rels': []}, 1, [])]
		points = [item['rec_id'] for item in search({'actual': False, 'object_id': 25, 'request': '', 'rels': []}, 1, [])]
		fc_geometries = feature_collection_by_geometry(group_id, 30, geometries, [30303], {})
		fc_points = feature_collection_by_geometry(group_id, 25, points, [50163, 50105], {})
