    ...
    >
      <template v-slot:label="{ item }">
        ...
        <v-hover v-slot="{ hover }">
          <v-text-field
            v-model="item === searchTreeItems[0] ? item.title : item.request"
            dense
            outlined
            hide-details
            :readonly="searchTreeItems[0] === item"
            color="teal"
            autocomplete="off"
...
export default {
  name: "searchRelationPage",
  components: {CreateTreeItemBtn, ChangeTreeItemBtn, FoundObjects},
  data: () => ({
    openObject: [],
  }),
  computed: {
    ...mapGetters(['searchRelationTreeGraph', 'foundObjects']),
    searchTreeItems: function () { return this.searchRelationTreeGraph ? [this.searchRelationTreeGraph] : [] },
  },
  methods: {
    ...mapActions(['setRootSearchTreeItem', 'changeSearchTreeItem', 'setActiveTool', 'addSearchTreeItem',
    'findRelationsOnServer', 'removeSearchTreeItem', 'setEditableObject', 'addToGraph']),
    selectObject(object) {
      this.addToGraph({payload: object})
    },
    changeObject(object) {
