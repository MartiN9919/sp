<template>
  <v-container fluid style="height: 100%" class="pa-0">
    <svg class="svg-demo" @contextmenu="activateRightClickMenu(RightClickMenuBackground, $event)">
      <circle @contextmenu.stop="activateRightClickMenu(RightClickMenuObject, $event)" r="60" cy="97" cx="497" stroke-width="2" stroke="#E4AF4C" fill="#F4D37D" />
    </svg>
    <right-click-menu
      v-if="showRightClickMenu"
      v-model="showRightClickMenu"
      :menuSettings="menuWatcher"
      @selectedItem="getClassifiersForObject"
    ></right-click-menu>
  </v-container>
</template>
...
export default {
  ...
  methods: {
    ...
    getClassifiersForObject (object) {
  ...
  created() {
    this.addListObjects({})
      .then(() => { this.RightClickMenuBackground.bodyRightClickMenu[0].items = this.listObjects})
  }
}
</script>
