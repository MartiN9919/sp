...
from   lib.db.const.const_dat          import DAT_SYS_KEY, DAT_OWNER
...
# проверить доступ одной записи-элементу obj_id.rec_id для группы пользователей group_id
io_permit = lambda write, io_org_item, obj_id, rec_id, group_id: \
    IO_PERMIT(io_org_item=io_org_item, obj_id=obj_id, ids=(int(rec_id),)).valid(write=write, group_id=group_id, rec_id=rec_id)
...
class IO_PERMIT():
    def __init__(self, io_org_item, obj_id, ids, ids_max_block=None):
        ...
        permits    = list(map(lambda x: (                                                           # begin           [(88, 10000, '23', '2019-01-02'), ...]
            ...
            int(x[2]),                                                                              # '23'         -> 23
            datetime.strptime(x[3] if x[3]!=None else '2000-01-01 00:00:00', '%Y-%m-%d %H:%M:%S'),  # '2019-01-02' -> datetime.datetime(2019, 1, 2, 0, 0)
        ...
        for item in permits:
            ...
            if   item[1] == DAT_SYS_KEY.NAME_OWNER_ADD_RW: groups_rw.add(item[2])
            elif item[1] == DAT_SYS_KEY.NAME_OWNER_ADD_RO: groups_ro.add(item[2])
            ...
            elif item[1] == DAT_SYS_KEY.NAME_OWNER_DEL:    groups_rw.discard(item[2]); groups_ro.discard(item[2])
            else: raise Exception('Error key_id: '+str(item))
        ...
        if DEBUG:
            print('\nIO_PERMIT.permits_rw')
            pprint(self.permits_rw)
            print('\nIO_PERMIT.permits_ro')
            pprint(self.permits_ro)
