...
from   lib.db.connect.connect_mysql import db_sql, db_connect, db_disconnect
from   lib.db.const.const_dat       import DAT_SYS_ID, DAT_OBJ_ROW
...
class IO_LIB_SQL():
    def __init__(self):
        self.connection = db_connect(database=CONNECT.DATA)
    ...
    def obj_rec_id_new(self, obj_id):
        ...
        self.__sql_exec__(sql="UPDATE "+DAT_SYS_ID.TABLE+" SET id = LAST_INSERT_ID(id + 1) WHERE "+DAT_SYS_ID.OBJ_ID+"="+str(obj_id), read=False)
    ...
    def obj_insert_col_one(self, data_pars):
        ...
        self.__sql_exec__(
            sql= \
                "INSERT IGNORE INTO "+data_pars.col_table+" "+\
    ...
    rec_insert_one = lambda self, table, equ: self.__sql_exec__(sql="INSERT IGNORE INTO "+table+" SET "+', '.join(equ), read=False)
    ...
    def obj_update_col_all(self, data_pars):
        if not data_pars.rec_id: raise Exception('Unknow rec_id: data_pars = '+str(data_pars))
        self.__sql_exec__(sql="UPDATE IGNORE "+data_pars.col_table+" SET "+', '.join(data_pars.col_equ_flat())+" WHERE id="+data_pars.rec_id+" LIMIT 1", read=False)
        return self.connection.affected_rows() == 1
    ...
    def _delete_(self, table, where, only_first=True):
        self.__sql_exec__(sql="DELETE FROM "+table+" WHERE "+where+(" LIMIT 1" if only_first else ""), read=False)
