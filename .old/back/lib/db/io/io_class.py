...
from   lib.db.const.const_dat      import DAT_SYS_OBJ, DAT_SYS_KEY, DAT_OWNER, DAT_REL
from   lib.db.io.pars.io_pars_data import IO_PARS_DATA
from   lib.db.io.pars.io_pars_keys import IO_PARS_KEYS
from   lib.db.io.pars.io_pars_rels import IO_PARS_RELS
from   lib.db.io.io_org_sql        import IO_ORG_SQL
from   lib.db.io.io_permit         import IO_PERMIT, io_permit
...
class IO():
    ...
    def get_obj(self, obj, keys=[], ids=[], ids_max_block=None, where_dop_row=[]):
        try:
            ...
            io_permit = IO_PERMIT(io_org_item=self.io_org[self.ORG_SQL], obj_id=keys_pars.obj_id, ids=ids, ids_max_block=ids_max_block) if not self.is_admin else None
            # выборка всех записей без проверки ограничений доступа
            for item in self.io_org[self.ORG_SQL].get_obj(keys_pars=keys_pars, ids=ids, ids_max_block=ids_max_block, where_dop_row=where_dop_row):
                # item = (31, 30302, 0), (31, 30303, 'Тест 2'), ...
                # только разрешенные записи
                if self.is_admin or io_permit.valid(write=False, group_id=self.group_id, rec_id=item[0]): yield item
        except Exception as e:
            # logging.getLogger(settings.PROJECT_LOG_MAIN).error(str(e))
            raise e
    ...
    def get_rel(self, keys=[], obj_rel_1=None, obj_rel_2=None, where_dop=[]):
        try:
            ...
            for item in self.io_org[self.ORG_SQL].get_rel(keys_pars=keys_pars, rels_pars=rels_pars, where_dop=where_dop):
