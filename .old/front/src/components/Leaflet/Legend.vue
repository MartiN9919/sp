<template>
  ...
  >
    ...
    <div
      v-for="(legend_color_item, legend_color_ind) in MAP_GET_ITEM_COLOR_LEGEND(options.hover_map_ind)"
      class="legend-color"
    >
      <div
...
export default {
  ...
  methods: {
    visible() {
      let block_color = (this.MAP_GET_ITEM_COLOR_LEGEND(this.options.hover_map_ind).length>0);
    ...
    prop_hint() {
      ...
      if ((this.options.hover_map_ind>-1) && (this.options.hover_feature_ind>-1))
        ret = this.MAP_GET_ITEM(this.options.hover_map_ind)[MAP_ITEM.FC].features[this.options.hover_feature_ind].properties.hint
    ...
    prop_val() {
      let ret;
      if ((this.options.hover_map_ind>-1) && (this.options.hover_feature_ind>-1))
        ret = this.MAP_GET_ITEM(this.options.hover_map_ind)[MAP_ITEM.FC].features[this.options.hover_feature_ind].properties.value
    ...
    class_select(legend_color_item) {
      ...
      let feature_color =
        ...
        ?this.MAP_GET_ITEM(this.options.hover_map_ind)[MAP_ITEM.FC].features[this.options.hover_feature_ind].color
        :undefined;
      return {
