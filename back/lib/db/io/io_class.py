...
from   lib.db.io.io_permit         import IO_PERMIT, permit
...
class IO():
    ...
    def get_obj(self, obj, keys=[], ids=[], ids_max_block=None, where_dop_row=[]):
        try:
            ...
            permit    = IO_PERMIT(io_org_item=self.io_org[self.ORG_SQL], obj_id=keys_pars.obj_id, ids=ids, ids_max_block=ids_max_block) if not self.is_admin else None
