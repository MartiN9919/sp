...
from   lib.db.const.const_dat import DAT_SYS_OBJ, DAT_OBJ_COL, DAT_OBJ_ROW, DAT_REL
from   lib.db.io.io_lib_sql   import IO_LIB_SQL
...
class IO_ORG_SQL():
    ...
    def _set_col_(self, data_pars):
        ...
        # rec_id известно:
            ...
            rec = self.io_sql.select(table=data_pars.col_table, select=['1'], where=['id='+data_pars.rec_id,]+data_pars.col_equ_flat(), only_first=True)
            if len(rec) > 0:
            ...
            rec = self.io_sql.select(table=data_pars.col_table, select=['1'], where=['id='+data_pars.rec_id,], only_first=True)
            if len(rec) > 0:
    ...
    def _set_row_(self, data_pars):
        ...
        if data_pars.obj_id != DAT_SYS_OBJ.ID_REL:
            ...
            # rec_id известно
                ...
                for ind, item in enumerate(data_pars.row_equ(is_null=True)):
                    rec = self.io_sql.select(table=data_pars.row_table, select=['1'], where=['id='+data_pars.rec_id,]+item, only_first=True)
                    if len(rec) > 0:
        ...
        # REL
            ...
            # проверить на повтор записи в БД
            rec = self.io_sql.select(table=data_pars.row_table, select=['1'], where=data_pars.row_equ_flat(is_null=True), only_first=True)
            if len(rec) > 0:
    ...
    def get_obj(self, keys_pars, ids=[], ids_max_block=None, where_dop_row=[]):
        def __read__(where_ids):
            ...
            where_keys_row = [] if keys_pars.all_key else [ DAT_OBJ_ROW.KEY_ID+' IN ('+','.join([str(i) for i in keys_pars.row_key])+')', ]
            ...
            if keys_pars.row_exist():
                for item in self.__get_obj_row__(keys_pars=keys_pars, where=where_ids+where_keys_row+where_dop_row): yield item
        ...
        if len(ids) > 0:
            ...
            for ids_ind in range(0, len(ids), ids_max_block):
                yield from __read__(where_ids=[ 'id IN ('+','.join([str(i) for i in ids[ids_ind:ids_ind+ids_max_block]])+')', ])
        # если нет ids
        else:
    ...
    def __get_obj_col__(self, keys_pars, where, only_first=False):
        ...
        for rec_item in self.io_sql.select(table=keys_pars.col_table, select=keys_pars.col_select, where=where, only_first=only_first):
            # одна col-запись (один id) - несколько col-ключей
            # col_key_item - очередной ключ
            for col_key_ind, col_key_item in enumerate(keys_pars.col_key, 1):                       # начать с 1 т.к. col_key[0]=id (см.select_fld)
                if rec_item[col_key_ind]==None: continue                                            # пустые значения пропустить, т.к. в col запись читается целиком
    ...
    def __get_obj_row__(self, keys_pars, where):
        ...
        for rec_item in self.io_sql.select(table=keys_pars.row_table, select=keys_pars.row_select, where=where, only_first=False):
            yield tuple(map(lambda x: str(x) if isinstance(x, datetime.datetime) else x, list(rec_item))) # дату-время в строку
    ...
    def get_rel(self, keys_pars, rels_pars, where_dop=[]):
        ...
        where += [] if keys_pars.all_key else [ DAT_REL.KEY_ID+' IN ('+','.join([str(i) for i in keys_pars.row_key])+')', ]

        # rec_item - очередная row-запись: ['key_id', 'dat', 'obj_id_1', 'rec_id_1', 'obj_id_2', 'rec_id_2']
        # вариант 1
        for rec_item in self.io_sql.select(table=keys_pars.row_table, select=keys_pars.row_select, where=where+rels_pars.equ_1, only_first=False):
            yield tuple(map(lambda x: str(x) if isinstance(x, datetime.datetime) else x, list(rec_item))) # дату-время в строку

        # вариант 2
        if len(rels_pars.equ_2) > 0:
            for rec_item in self.io_sql.select(table=keys_pars.row_table, select=keys_pars.row_select, where=where+rels_pars.equ_2, only_first=False):
                yield tuple(map(lambda x: str(x) if isinstance(x, datetime.datetime) else x, list(rec_item))) # дату-время в строку
