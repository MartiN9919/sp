...
from   lib.db.const.const_dat import DAT_SYS_OBJ, DAT_SYS_KEY, DAT_OBJ_ROW, DAT_OBJ_COL, DAT_REL
...
class IO_PARS_DATA(dict):
    ...
    rec_id = property()
    @rec_id.getter
    def rec_id(self): return self[self.REC_ID]
    @rec_id.setter
    ...
    # EXIST
    ###########################################
    def col_exist(self): return len(self.col_dic) > 0
    def row_exist(self): return len(self.row_dic) > 0
    ...
    def val(self, type, val):
        if   type in [DAT_SYS_KEY.TYPE_STR, DAT_SYS_KEY.TYPE_DATA]: ret = "'"+val+"'"
        elif type==DAT_SYS_KEY.TYPE_GEOMETRY:                       ret = "ST_GeomFromGeoJson('"+val+"')"
        else:                                                       ret = str(val)
        return ret
    ...
    equ_item = lambda self, item_dic, is_null: [ item + (('='+item_dic[item]) if (not is_null) or item_dic[item] != 'null' else ' is null') for item in item_dic ]

    # obj_col:                [['type=4'], ["path='test/new2'"]]
    ...
    __equ__ = lambda self, dic, is_null: [ self.equ_item(item_dic=item_dic, is_null=is_null) for item_dic in dic ]       # INSERT - is_null = True
    ...
    def __init_rel__(self, data, add):
        ...
        for data_item in data:
            ...
            if data_key == DAT_REL.KEY_ID:
                ...
                if add.get(self.ADD_SYS_KEY, False): self.row_key.append(DAT_SYS_KEY.DUMP.get_rec(obj_id=self.obj_id, id=key_id))
