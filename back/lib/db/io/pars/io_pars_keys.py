...
from   lib.db.const.const_dat import DAT_SYS_OBJ, DAT_SYS_KEY, DAT_OBJ_COL, DAT_OBJ_ROW, DAT_REL
...
class IO_PARS_KEYS(dict):
    ...
    # EXIST
    ###########################################
    def col_exist(self): return len(self.col_key) > 0
    def row_exist(self): return (len(self.row_key) > 0) or self.all_key
    ...
    def __init_rel__(self, keys=[]):
        self.row_table  = self[self.ROW_TABLE ] = DAT_REL.TABLE
